...
def load_gpx_file(path: str) -> gpxpy.gpx.GPX | None:
    """Read and parse a GPX file from the given path, returning a GPX object or None on error."""
    try:
...
def plot_file(file_name: str, name: str):
    gpx = load_gpx_file(os.path.join(os.path.dirname(os.path.abspath(__file__)), file_name))
